import type { JwtResponse, Task, TaskStatus } from "./types";
...
export class ApiError extends Error {
  ...
  constructor(
    message: string,
    status: number,
    code: "HTTP_ERROR" | "NETWORK_ERROR" | "INVALID_RESPONSE" = "HTTP_ERROR",
  ) {
    super(message);
    this.name = "ApiError";
    this.status = status;
...
function parseJwtResponse(value: unknown): JwtResponse {
  if (
    !isRecord(value) ||
...
export const taskApi = {
  list: (signal?: AbortSignal) =>
    apiRequest("/tasks", { signal }, parseTaskList),

  create: (title: string, description: string, signal?: AbortSignal) =>
    apiRequest(
      "/tasks",
      {
        method: "POST",
