...
docker build -t "$REGISTRY/$USERNAME/$BACKEND_IMAGE:v1" \
  -f services/backend/Dockerfile services/backend

docker push "$REGISTRY/$USERNAME/$BACKEND_IMAGE:v1"

docker build -t "$REGISTRY/$USERNAME/$FRONTEND_IMAGE:v1" \
  -f services/frontend/Dockerfile services/frontend

docker push "$REGISTRY/$USERNAME/$FRONTEND_IMAGE:v1"
